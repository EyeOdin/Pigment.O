...
class PigmentODocker(DockWidget):
    ...
    def Eraser(self, switch):
        if ((self.canvas() is not None) and (self.canvas().view() is not None)):
            ...
            if state is False:  # Eraser is OFF
                ...
                elif switch == "OFF":  # do nothing
                    pass
            elif state is True:  # Eraser is ON
                if switch == "ON":  # do nothing
                    pass
                elif switch == "OFF":  # Switch OFF
                    kritaEraserAction.trigger()
    ...
    def Pigment_Color(self, mode):
        # Change Color Deactivates Eraser
        self.Eraser("OFF")
        # Change Color
        if mode == "RGB":
            # Original
    ...
    def Pigment_2_Krita(self, rgb1, rgb2, rgb3, hsv1, hsv2, hsv3):
        ...
        if ((self.canvas() is not None) and (self.canvas().view() is not None)):
            # Check if the Eraser is turnned ON
            Application.action("KritaShape/KisToolBrush").trigger()
            kritaEraserAction = Application.action("erase_action")
