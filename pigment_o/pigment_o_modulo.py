...
class Panel_IMG(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.analyze_display == True: # Text
            ...
            painter.drawRect(0, 0, self.qimage_w, self.qimage_h)
            painter.setPen(QColor("#d4d4d4"))
            font_type = 'Lucida'
            font_size = 10
            painter.setFont(QFont(font_type, font_size, QtGui.QFont.Bold))
            painter.drawText(event.rect(), Qt.AlignHCenter|Qt.AlignVCenter, "SCANNING\nIMAGE")
class Thread_IMG(QThread):
    ...
    def run(self):
        ...
        self.SIGNAL_SCAN_MAX.emit((self.width-1) * (self.height-1))
        try:
