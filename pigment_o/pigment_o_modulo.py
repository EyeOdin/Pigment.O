...
class Luma_Lock(QWidget):
    SIGNAL_LUMA_LOCK = QtCore.pyqtSignal(int)

    # Init
    def __init__(self, parent):
        super(Luma_Lock, self).__init__(parent)
        self.aaa_lock = False
    ...
    # Relay
    def Update(self, aaa_lock, gray_natural, gray_contrast, width, height):
...
class Panel_YUV(QWidget):
    ...
    def Update_Panel(self, yuv, cor1, cor2, cor3, cor4, cor5, cor6, width, height, hex, zoom):
        ...
        self.cor1 = [cor1[0], cor1[1], cor1[2]]
        self.cor2 = [cor2[0], cor2[1], cor2[2]]
        self.cor3 = [cor3[0], cor3[1], cor3[2]]

        self.cor4 = [cor4[0], cor4[1], cor4[2]]
        self.cor5 = [cor5[0], cor5[1], cor5[2]]
        self.cor6 = [cor6[0], cor6[1], cor6[2]]
    ...
    def drawColors(self, event):
        ...
        # painter.setPen(QtCore.Qt.NoPen)
        #
        # # Gradient Blue Component U
        # red = QLinearGradient(0, 0, 0, self.panel_height)
        # red.setColorAt(0.000, QColor(self.cor1[0]*255, self.cor1[1]*255, self.cor1[2]*255, 255))
        # red.setColorAt(0.500, QColor(self.cor2[0]*255, self.cor2[1]*255, self.cor2[2]*255, 255))
        # red.setColorAt(1.000, QColor(self.cor3[0]*255, self.cor3[1]*255, self.cor3[2]*255, 255))
        # painter.setBrush(QBrush(red))
        # painter.drawRect(0,0, self.panel_width, self.panel_height)
        #
        # # Gradient Red Component V
        ...
        # painter.setCompositionMode(QPainter.CompositionMode_SourceOver)
        #
        # blue = QLinearGradient(0, 0, 0, self.panel_height)
        # blue.setColorAt(0.000, QColor(self.cor4[0]*255, self.cor4[1]*255, self.cor4[2]*255, 255))
        # blue.setColorAt(0.500, QColor(self.cor5[0]*255, self.cor5[1]*255, self.cor5[2]*255, 255))
        # blue.setColorAt(1.000, QColor(self.cor6[0]*255, self.cor6[1]*255, self.cor6[2]*255, 255))
        # painter.setBrush(QBrush(blue))
        # painter.drawRect(0,0, self.panel_width, self.panel_height)
        #
        # # Lines
        # painter.setCompositionMode(QPainter.CompositionMode_SourceOver)
        # painter.setPen(QPen(QColor(self.color_dark), 2, Qt.SolidLine, Qt.SquareCap, Qt.MiterJoin))
        # lines = QPainterPath()
        ...
        # self.luminosity = "601"
        # if self.luminosity == "601":
        #     self.luma_pr = 1.402
        #     self.luma_pb = 1.772
        # if self.luminosity == "709":
        #     self.luma_pr = 1.5748
        #     self.luma_pb = 1.8556
        #
        # # Verify UVD D change with previous selected color to see if it is worth changing
        # self.image = QImage(self.panel_width, self.panel_height, QImage.Format_RGB32)
        # # Cycle through Pixels
        #
        # for w in range(self.panel_width):
        #     for h in range(self.panel_height):
        #         # Consider pixel location and its location color
        #         y = self.yuv[0]
        #         u = w / self.panel_width
        #         v = 1 - (h / self.panel_height)
        #         # Convert YUV to RGB
        #         # u = u - 0.5 # slider adjustment offset
        #         # v = v - 0.5 # slider adjustment offset
        #         r = self.luma_pr * u + y
        ...
        # painter.setPen(QtCore.Qt.NoPen)
        #
        # painter.drawPixmap(0,0, self.panel_width,self.panel_height, QPixmap(self.image))
...
class Panel_HUE_Circle(QWidget):
    ...
    # Update Normal
    def Update_Panel(self, wheel, harmony_render, harmony_index, angle, hue, width, height, gray_natural, gray_contrast):
        # Color
        ...
        self.harmony_index = harmony_index # Monochromatic Complemantary Analogous Split Complemantary Double Split Complemantary
    ...
    # Update Harmony
    def Update_Harmony_1(self, wheel, harmony_render, harmony_index, angle, color, width, height, gray_natural, gray_contrast):
        # Color
        self.wheel = wheel # CMY RYB
        # Harmony
        self.harmony_render = harmony_render # COLOR HARMONY
    ...
    def Update_Harmony_2(self, wheel, harmony_render, harmony_index, angle, color, width, height, gray_natural, gray_contrast):
        # Color
        self.wheel = wheel # CMY RYB
        # Harmony
        self.harmony_render = harmony_render # COLOR HARMONY
    ...
    def Update_Harmony_3(self, wheel, harmony_render, harmony_index, angle, color, width, height, gray_natural, gray_contrast):
        # Color
        self.wheel = wheel # CMY RYB
        # Harmony
        self.harmony_render = harmony_render # COLOR HARMONY
    ...
    def Update_Harmony_4(self, wheel, harmony_render, harmony_index, angle, color, width, height, gray_natural, gray_contrast):
        # Color
        self.wheel = wheel # CMY RYB
        # Harmony
        self.harmony_render = harmony_render # COLOR HARMONY
    ...
    def Update_Harmony_5(self, wheel, harmony_render, harmony_index, angle, color, width, height, gray_natural, gray_contrast):
        # Color
        self.wheel = wheel # CMY RYB
        # Harmony
        self.harmony_render = harmony_render # COLOR HARMONY
...
class Panel_GAM_Polygon(QWidget):
    ...
    def drawColors(self, event):
        ...
        painter.setPen(QPen(QColor(self.gray_natural), 5, Qt.SolidLine, Qt.SquareCap, Qt.MiterJoin))
        if self.wheel == "CMY":
            # painter.drawLine(self.panel_width*0,self.panel_height*0.5, self.panel_width*1,self.panel_height*0.5)
            # painter.drawLine(self.panel_width*0.21132,self.panel_height*0, self.panel_width*0.78868,self.panel_height*1)
            # painter.drawLine(self.panel_width*0.78868,self.panel_height*0, self.panel_width*0.21132,self.panel_height*1)
...
class Clicks(QWidget):
    ...
    def drawColors(self, event):
        # Text
        self.text = (
            "S "+str(round(self.size,0))+"\n"+
            "O "+str(round(self.opacity*100,0))+"\n"+
            "F "+str(round(self.flow*100,0))
            )
        # QPainter Start
        painter = QPainter()
        painter.begin(self)
