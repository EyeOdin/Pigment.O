...
class Panel_IMG(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Images Sizes
        if self.display == True: # Image File
            img_w = self.qimage.width()
            img_h = self.qimage.height()
        elif self.display == False: # Drop Files Here
            img_w = self.default.width()
            img_h = self.default.height()
        # Render
        if self.display == True:
            if self.frame == True: # Frame
                var_w = self.panel_width / img_w
                var_h = self.panel_height / img_h
                if var_w <= var_h:
                    size = var_w
                if var_w > var_h:
                    size = var_h
                ...
                ox = (self.panel_width * 0.5) - (img_w * self.focus_x * self.zoom)
                oy = (self.panel_height * 0.5) - (img_h * self.focus_y * self.zoom)
                painter.drawImage(ox, oy, qimage.scaled(img_w * self.zoom, img_h * self.zoom, Qt.KeepAspectRatio, Qt.SmoothTransformation))
        else: # Drop File Here
            var_w = self.panel_width / img_w
            var_h = self.panel_height / img_h
            if var_w <= var_h:
