...
class Panel_IMG(QWidget):
    ...
    SIGNAL_IMG_PATH = QtCore.pyqtSignal(str)
    ...
    def mouseCursor(self, event):
        ...
        if self.value_y >= self.panel_height:
            self.value_y = self.panel_height
        # Operations Left
        if ((event.buttons() == QtCore.Qt.LeftButton or event.buttons() == QtCore.Qt.MiddleButton or event.buttons() == QtCore.Qt.RightButton) and event.modifiers() == QtCore.Qt.NoModifier): # Pick Color
        ...
        if (event.buttons() == QtCore.Qt.LeftButton and event.modifiers() == QtCore.Qt.AltModifier):
            self.Context_Clean()
        # Operations Right
        if (event.buttons() == QtCore.Qt.RightButton and event.modifiers() == QtCore.Qt.ShiftModifier): # Reset
            self.Context_Reset()
        if (event.buttons() == QtCore.Qt.RightButton and event.modifiers() == QtCore.Qt.ControlModifier): # Zoom
            delta = 5
            ...
            if self.zoom <= 0:
                self.zoom = 0
        if (event.buttons() == QtCore.Qt.RightButton and event.modifiers() == QtCore.Qt.AltModifier): # Offset
    ...
    def dropEvent(self, event):
        if event.mimeData().hasImage:
            ...
            # When recieving from Photobash Images, this method gives size(zero,zero) when the source is the OS
            self.qimage = QImage(event.mimeData().imageData())
            self.qimage_w = self.qimage.width()
            self.qimage_h = self.qimage.height()
            # Detect anormality of OS source and try standard method to replace information of size(zero,zero)
            ...
            self.Context_Reset()
            self.Context_Frame()
            self.SIGNAL_IMG_PATH.emit(self.path)
            event.accept()
        else:
            event.ignore()
        self.update()
    ...
    def contextMenuEvent(self, event):
        if event.modifiers() == QtCore.Qt.NoModifier:
            ...
            if self.display == True:
                ...
                cmenu_locate = cmenu.addAction("Locate")
    ...
    def Context_Scan(self):
        ...
        self.thread.start()
    def Context_Locate(self):
        # Open Location
        operating_system = str(QSysInfo.kernelType()) # WINDOWS=winnt & LINUX=linux
        if operating_system == "winnt": # Windows
            FILEBROWSER_PATH = os.path.join(os.getenv('WINDIR'), 'explorer.exe')
            subprocess.run([FILEBROWSER_PATH, '/select,', os.path.normpath(self.path)])
...
class Thread_IMG(QThread):
    ...
    def Variables(self, qimage):
        self.qimage = qimage
        # self.qimage = qimage.scaled(100, 100, Qt.KeepAspectRatio, Qt.FastTransformation)
    ...
    def run(self):
        ...
        try:
            ...
            # Read Colors
            max_value = (self.width-1) * (self.height-1)
            self.SIGNAL_SCAN_VAL.emit(0)
            self.SIGNAL_SCAN_MAX.emit(max_value)
            self.SIGNAL_SCAN_TEXT.emit("SCANNING\nIMAGE")

            # ct = self.qimage.colorTable()
            # QtCore.qDebug(str(ct[0]))
            ...
            for h in range(0, self.height-1, cycle):
                for w in range(0, self.width-1, cycle):
                    # Progress bar
                    self.SIGNAL_SCAN_VAL.emit( (self.width * h) + w )
                    # Pixel Information
                    ...
                    for i in range(0, length):
                        lrgb = [lista[i][2], lista[i][3], lista[i][4]]
                        if lrgb == rgb:
                            lista[i][1] = lista[i][1] + aaa
                            app = False
                            break
                    if app == True:
                        lista.append(cor)
            ...
            QtCore.qDebug(str(lista))
            QtCore.qDebug(str(length))


        except:
            pass

        self.SIGNAL_SCAN_VAL.emit(max_value)
        self.SIGNAL_COLORS.emit(lista)
