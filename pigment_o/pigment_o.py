...
class PigmentODocker(DockWidget):
    ...
    def Object(self):
        ...
        self.layout.panel_layers.setMouseTracking(True)
        self.layout.panel_layers.setTabletTracking(True)
        self.layout.panel_layers.installEventFilter(self)
    ...
    def Signal_OBJ(self, SIGNAL_OBJECT):
        # Geometry
        # self.update()
        width = self.layout.panel_layers.width()
        height = self.layout.panel_layers.height()
        pixmap = self.layout.panel_layers.grab(QRect(QPoint(0, 0), QPoint(width, height)))
