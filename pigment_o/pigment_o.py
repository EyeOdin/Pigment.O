...
class PigmentODocker(DockWidget):
    ...
    def User_Interface(self):
        ...
        # Shader Range
        self.layout.object_value.setMinimum(0)
        self.layout.light_value.setMinimum(0)
        self.layout.ambient_value.setMinimum(0)
        self.layout.background_value.setMinimum(0)
        self.layout.object_value.setMaximum(kritaOBJ)
        self.layout.light_value.setMaximum(kritaLIG)
        self.layout.ambient_value.setMaximum(kritaAMB)
        self.layout.background_value.setMaximum(kritaBGC)
    def Setup(self):
        # Module SOF
    ...
    def Shader(self):
        # Object
        self.layout.object_live.clicked.connect(self.Shader_Object)
        self.object_color_input = Clicks(self.layout.object_color_input)
        self.object_color_input.SIGNAL_APPLY.connect(self.Shader_Object_APPLY)
        self.object_color_input.SIGNAL_SAVE.connect(self.Shader_Object_SAVE)
        self.object_color_input.SIGNAL_CLEAN.connect(self.Shader_Object_CLEAN)
        self.object_slider = Channel_Linear(self.layout.object_slider)
        self.object_slider.Setup("NEU")
        self.object_slider.SIGNAL_VALUE.connect(self.Object_Slider)
        self.layout.object_value.editingFinished.connect(self.Object_Value)
        # Light
        self.layout.light_live.clicked.connect(self.Shader_Light)
        self.light_color_input = Clicks(self.layout.light_color_input)
        self.light_color_input.SIGNAL_APPLY.connect(self.Shader_Light_APPLY)
        self.light_color_input.SIGNAL_SAVE.connect(self.Shader_Light_SAVE)
        self.light_color_input.SIGNAL_CLEAN.connect(self.Shader_Light_CLEAN)
        self.light_slider = Channel_Linear(self.layout.light_slider)
        self.light_slider.Setup("NEU")
        self.light_slider.SIGNAL_VALUE.connect(self.Light_Slider)
        self.layout.light_value.editingFinished.connect(self.Light_Value)
        # Ambient
        self.layout.ambient_live.clicked.connect(self.Shader_Ambient)
        self.ambient_color_input = Clicks(self.layout.ambient_color_input)
        self.ambient_color_input.SIGNAL_APPLY.connect(self.Shader_Ambient_APPLY)
        self.ambient_color_input.SIGNAL_SAVE.connect(self.Shader_Ambient_SAVE)
        self.ambient_color_input.SIGNAL_CLEAN.connect(self.Shader_Ambient_CLEAN)
        self.ambient_slider = Channel_Linear(self.layout.ambient_slider)
        self.ambient_slider.Setup("NEU")
        self.ambient_slider.SIGNAL_VALUE.connect(self.Ambient_Slider)
        self.layout.ambient_value.editingFinished.connect(self.Ambient_Value)
        # Background
        self.layout.background_live.clicked.connect(self.Shader_Background)
        self.background_color_input = Clicks(self.layout.background_color_input)
        self.background_color_input.SIGNAL_APPLY.connect(self.Shader_Background_APPLY)
        self.background_color_input.SIGNAL_SAVE.connect(self.Shader_Background_SAVE)
        self.background_color_input.SIGNAL_CLEAN.connect(self.Shader_Background_CLEAN)
        self.background_slider = Channel_Linear(self.layout.background_slider)
        self.background_slider.Setup("NEU")
        self.background_slider.SIGNAL_VALUE.connect(self.Background_Slider)
        self.layout.background_value.editingFinished.connect(self.Background_Value)
        # Material Display
        img = QPixmap(os.path.dirname(os.path.realpath(__file__)) + "/Shader/A_03.png")
        self.layout.render.setPixmap(img)
        self.color_background = ["True",0,0,0]
        # SVG Shader
        # self.sphere = QtSvg.QSvgWidget(self.layout.render)
        # self.array_sphere_diffuse = self.SVG_Sphere_Diffuse([0,0,0],[0.1,0.1,0.1],[0.2,0.2,0.2],[0.3,0.3,0.3],[0.4,0.4,0.4],[0.5,0.5,0.5],1)
        # self.sphere.load(self.array_sphere_diffuse)
        # self.sphere_size = 100
        # self.sphere.setGeometry(QtCore.QRect(0, 0, self.sphere_size * 2, self.sphere_size * 2))
        # self.sphere.move((self.layout.render.width() / 2)- self.sphere_size, (self.layout.render.height() / 2) - self.sphere_size)
        # self.sphere.setStyleSheet(bg_unseen)
        # Output Display
        self.output_1_display = Clicks(self.layout.output_1_display)
        self.output_2_display = Clicks(self.layout.output_2_display)
        self.output_3_display = Clicks(self.layout.output_3_display)
        self.output_4_display = Clicks(self.layout.output_4_display)
        self.output_5_display = Clicks(self.layout.output_5_display)
        self.output_1_display.SIGNAL_APPLY.connect(self.Shader_Output_1_APPLY)
        self.output_2_display.SIGNAL_APPLY.connect(self.Shader_Output_2_APPLY)
        self.output_3_display.SIGNAL_APPLY.connect(self.Shader_Output_3_APPLY)
        self.output_4_display.SIGNAL_APPLY.connect(self.Shader_Output_4_APPLY)
        self.output_5_display.SIGNAL_APPLY.connect(self.Shader_Output_5_APPLY)
        # Output Values
        self.output_1_values = Clicks(self.layout.output_1_values)
        self.output_2_values = Clicks(self.layout.output_2_values)
        self.output_3_values = Clicks(self.layout.output_3_values)
        self.output_4_values = Clicks(self.layout.output_4_values)
        self.output_5_values = Clicks(self.layout.output_5_values)
    ...
    def Menu_Panel(self):
        ...
        if panel == "PANEL":
            ...
            self.layout.panel_render.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        if panel == "HSV":
        ...
        if panel == "SPHERE":
            ...
            self.layout.panel_hsv_bg.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
            self.layout.panel_hsl_bg.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
            self.layout.panel_render.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.Settings_Save()
    def Panel_Shrink(self):
        ...
        self.layout.panel_hsl_bg.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        self.layout.panel_render.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
    def Shader_Scale(self):
        # Vertical Spacing
        self.layout.spacer_shader_input_1.setMinimumHeight(unit)
        self.layout.spacer_shader_input_2.setMinimumHeight(unit)
        self.layout.spacer_shader_input_3.setMinimumHeight(unit)
        self.layout.spacer_shader_output_1.setMinimumHeight(unit)
        self.layout.spacer_shader_output_2.setMinimumHeight(unit)
        self.layout.spacer_shader_output_3.setMinimumHeight(unit)
        self.layout.spacer_shader_output_4.setMinimumHeight(unit)
        self.layout.spacer_shader_output_4.setMinimumHeight(unit)
        # Size Policy
        self.layout.shader_input.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        self.layout.shader_display.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.layout.shader_output.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        # Object
        self.layout.object_live.setMinimumHeight(ui_min_1)
        self.layout.object_live.setMaximumHeight(ui_max_1)
        self.layout.object_color_input.setMinimumHeight(ui_min_1)
        self.layout.object_color_input.setMaximumHeight(ui_max_1)
        self.layout.object_slider.setMinimumHeight(ui_min_1)
        self.layout.object_slider.setMaximumHeight(ui_max_1)
        self.layout.object_value.setMinimumHeight(ui_min_1)
        self.layout.object_value.setMaximumHeight(ui_max_1)
        # Light
        self.layout.light_live.setMinimumHeight(ui_min_1)
        self.layout.light_live.setMaximumHeight(ui_max_1)
        self.layout.light_color_input.setMinimumHeight(ui_min_1)
        self.layout.light_color_input.setMaximumHeight(ui_max_1)
        self.layout.light_slider.setMinimumHeight(ui_min_1)
        self.layout.light_slider.setMaximumHeight(ui_max_1)
        self.layout.light_value.setMinimumHeight(ui_min_1)
        self.layout.light_value.setMaximumHeight(ui_max_1)
        # Ambient
        self.layout.ambient_live.setMinimumHeight(ui_min_1)
        self.layout.ambient_live.setMaximumHeight(ui_max_1)
        self.layout.ambient_color_input.setMinimumHeight(ui_min_1)
        self.layout.ambient_color_input.setMaximumHeight(ui_max_1)
        self.layout.ambient_slider.setMinimumHeight(ui_min_1)
        self.layout.ambient_slider.setMaximumHeight(ui_max_1)
        self.layout.ambient_value.setMinimumHeight(ui_min_1)
        self.layout.ambient_value.setMaximumHeight(ui_max_1)
        # Background
        self.layout.background_live.setMinimumHeight(ui_min_1)
        self.layout.background_live.setMaximumHeight(ui_max_1)
        self.layout.background_color_input.setMinimumHeight(ui_min_1)
        self.layout.background_color_input.setMaximumHeight(ui_max_1)
        self.layout.background_slider.setMinimumHeight(ui_min_1)
        self.layout.background_slider.setMaximumHeight(ui_max_1)
        self.layout.background_value.setMinimumHeight(ui_min_1)
        self.layout.background_value.setMaximumHeight(ui_max_1)
        # Output
        self.layout.output_1_display.setMinimumHeight(ui_max_1)
        self.layout.output_1_display.setMaximumHeight(ui_tip)
        self.layout.output_1_values.setMinimumHeight(ui_tip)
        self.layout.output_1_values.setMaximumHeight(ui_tip)
        self.layout.output_2_display.setMinimumHeight(ui_max_1)
        self.layout.output_2_display.setMaximumHeight(ui_tip)
        self.layout.output_2_values.setMinimumHeight(ui_tip)
        self.layout.output_2_values.setMaximumHeight(ui_tip)
        self.layout.output_3_display.setMinimumHeight(ui_max_1)
        self.layout.output_3_display.setMaximumHeight(ui_tip)
        self.layout.output_3_values.setMinimumHeight(ui_tip)
        self.layout.output_3_values.setMaximumHeight(ui_tip)
        self.layout.output_4_display.setMinimumHeight(ui_max_1)
        self.layout.output_4_display.setMaximumHeight(ui_tip)
        self.layout.output_4_values.setMinimumHeight(ui_tip)
        self.layout.output_4_values.setMaximumHeight(ui_tip)
        self.layout.output_5_display.setMinimumHeight(ui_max_1)
        self.layout.output_5_display.setMaximumHeight(ui_tip)
        self.layout.output_5_values.setMinimumHeight(ui_tip)
        self.layout.output_5_values.setMaximumHeight(ui_tip)
        # Margins
        self.layout.shader_input.setContentsMargins(ui_vspacer, ui_vspacer, 0, ui_vspacer)
        self.layout.shader_output.setContentsMargins(0, ui_vspacer, 0, 0)
    def Shader_Shrink(self):
        # Spacing
        self.layout.spacer_shader_input_1.setMinimumHeight(zero)
        self.layout.spacer_shader_input_2.setMinimumHeight(zero)
        self.layout.spacer_shader_input_3.setMinimumHeight(zero)
        self.layout.spacer_shader_output_1.setMinimumHeight(zero)
        self.layout.spacer_shader_output_2.setMinimumHeight(zero)
        self.layout.spacer_shader_output_3.setMinimumHeight(zero)
        self.layout.spacer_shader_output_4.setMinimumHeight(zero)
        self.layout.spacer_shader_output_4.setMinimumHeight(zero)
        # Nullify Live
        self.layout.object_live.setChecked(0)
        self.layout.light_live.setChecked(0)
        self.layout.ambient_live.setChecked(0)
        self.layout.background_live.setChecked(0)
        # Size Policy
        self.layout.shader_input.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Ignored)
        self.layout.shader_display.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Ignored)
        self.layout.shader_output.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Ignored)
        # Object
        self.layout.object_live.setMinimumHeight(zero)
        self.layout.object_live.setMaximumHeight(zero)
        self.layout.object_color_input.setMinimumHeight(zero)
        self.layout.object_color_input.setMaximumHeight(zero)
        self.layout.object_slider.setMinimumHeight(zero)
        self.layout.object_slider.setMaximumHeight(zero)
        self.layout.object_value.setMinimumHeight(zero)
        self.layout.object_value.setMaximumHeight(zero)
        # Light
        self.layout.light_live.setMinimumHeight(zero)
        self.layout.light_live.setMaximumHeight(zero)
        self.layout.light_color_input.setMinimumHeight(zero)
        self.layout.light_color_input.setMaximumHeight(zero)
        self.layout.light_slider.setMinimumHeight(zero)
        self.layout.light_slider.setMaximumHeight(zero)
        self.layout.light_value.setMinimumHeight(zero)
        self.layout.light_value.setMaximumHeight(zero)
        # Ambient
        self.layout.ambient_live.setMinimumHeight(zero)
        self.layout.ambient_live.setMaximumHeight(zero)
        self.layout.ambient_color_input.setMinimumHeight(zero)
        self.layout.ambient_color_input.setMaximumHeight(zero)
        self.layout.ambient_slider.setMinimumHeight(zero)
        self.layout.ambient_slider.setMaximumHeight(zero)
        self.layout.ambient_value.setMinimumHeight(zero)
        self.layout.ambient_value.setMaximumHeight(zero)
        # Background
        self.layout.background_live.setMinimumHeight(zero)
        self.layout.background_live.setMaximumHeight(zero)
        self.layout.background_color_input.setMinimumHeight(zero)
        self.layout.background_color_input.setMaximumHeight(zero)
        self.layout.background_slider.setMinimumHeight(zero)
        self.layout.background_slider.setMaximumHeight(zero)
        self.layout.background_value.setMinimumHeight(zero)
        self.layout.background_value.setMaximumHeight(zero)
        # Output
        self.layout.output_1_display.setMinimumHeight(zero)
        self.layout.output_1_display.setMaximumHeight(zero)
        self.layout.output_1_values.setMinimumHeight(zero)
        self.layout.output_1_values.setMaximumHeight(zero)
        self.layout.output_2_display.setMinimumHeight(zero)
        self.layout.output_2_display.setMaximumHeight(zero)
        self.layout.output_2_values.setMinimumHeight(zero)
        self.layout.output_2_values.setMaximumHeight(zero)
        self.layout.output_3_display.setMinimumHeight(zero)
        self.layout.output_3_display.setMaximumHeight(zero)
        self.layout.output_3_values.setMinimumHeight(zero)
        self.layout.output_3_values.setMaximumHeight(zero)
        self.layout.output_4_display.setMinimumHeight(zero)
        self.layout.output_4_display.setMaximumHeight(zero)
        self.layout.output_4_values.setMinimumHeight(zero)
        self.layout.output_4_values.setMaximumHeight(zero)
        self.layout.output_5_display.setMinimumHeight(zero)
        self.layout.output_5_display.setMaximumHeight(zero)
        self.layout.output_5_values.setMinimumHeight(zero)
        self.layout.output_5_values.setMaximumHeight(zero)
        # Margins
        self.layout.shader_input.setContentsMargins(0, 0, 0, 0)
        self.layout.shader_output.setContentsMargins(0, 0, 0, 0)
    def Menu_SOF(self):
    ...
    def Pigment_Display(self):
        ...
        # SHADER Color
        if panel == "SPHERE":
            sss_sha_1 = str(self.NEU_Gradient(self.sha_1))
            sss_sha_2 = str(self.NEU_Gradient(self.sha_2))
            sss_sha_3 = str(self.NEU_Gradient(self.sha_3))
            sss_sha_4 = str(self.NEU_Gradient(self.sha_4))
        else:
            sss_sha_1 = bg_alpha
            sss_sha_2 = bg_alpha
            sss_sha_3 = bg_alpha
        ...
        self.layout.object_slider.setStyleSheet(sss_sha_1)
        self.layout.light_slider.setStyleSheet(sss_sha_2)
        self.layout.ambient_slider.setStyleSheet(sss_sha_3)
        self.layout.background_slider.setStyleSheet(sss_sha_4)
    ...
    # Shader ###################################################################
    def Shader_Object(self):
        live = self.layout.object_live.isChecked()
        if live == True:
            # Auto Exclusive
            self.layout.light_live.setChecked(0)
            self.layout.ambient_live.setChecked(0)
            ...
            self.Color_RGB(self.color_object[1], self.color_object[2], self.color_object[3])
    def Shader_Light(self):
        live = self.layout.light_live.isChecked()
        if live == True:
            # Auto Exclusive
            self.layout.object_live.setChecked(0)
            self.layout.ambient_live.setChecked(0)
            self.layout.background_live.setChecked(0)
            # Apply Color before Live
            self.Color_RGB(self.color_light[1], self.color_light[2], self.color_light[3])
    def Shader_Ambient(self):
        live = self.layout.ambient_live.isChecked()
        if live == True:
            # Auto Exclusive
            self.layout.object_live.setChecked(0)
            self.layout.light_live.setChecked(0)
            self.layout.background_live.setChecked(0)
            # Apply Color before Live
            self.Color_RGB(self.color_ambient[1], self.color_ambient[2], self.color_ambient[3])
    def Shader_Background(self):
        live = self.layout.background_live.isChecked()
        if live == True:
            # Auto Exclusive
            self.layout.object_live.setChecked(0)
            self.layout.light_live.setChecked(0)
            self.layout.ambient_live.setChecked(0)
            # Apply Color before Live
            self.Color_RGB(self.color_background[1], self.color_background[2], self.color_background[3])

    def Shader_Object_Live(self):
        live = self.layout.object_live.isChecked()
        if live == True:
            self.Shader_Object_SAVE(0)
            self.Pigment_Display_Release(0)
    def Shader_Light_Live(self):
        live = self.layout.light_live.isChecked()
        if live == True:
            self.Shader_Light_SAVE(0)
            self.Pigment_Display_Release(0)
    def Shader_Ambient_Live(self):
        live = self.layout.ambient_live.isChecked()
        if live == True:
            self.Shader_Ambient_SAVE(0)
            self.Pigment_Display_Release(0)
    def Shader_Background_Live(self):
        live = self.layout.background_live.isChecked()
        if live == True:
            self.Shader_Background_SAVE(0)
            self.Pigment_Display_Release(0)

    def Shader_Object_APPLY(self, SIGNAL_APPLY):
        if self.color_object[0] == "True":
            self.Color_RGB(self.color_object[1], self.color_object[2], self.color_object[3])
            self.Pigment_Display_Release(0)
            self.UVD_Release()
    def Shader_Object_SAVE(self, SIGNAL_SAVE):
        # Background
        self.layout.object_color.setStyleSheet("background-color: rgba(56, 56, 56, 255);")
        # Save Values
        self.color_object = ["True", self.rgb_1, self.rgb_2, self.rgb_3, self.layout.object_value.value()]
        # Input
        color1 = str("background-color: rgb(%f, %f, %f);" % (self.color_object[1]*hexRGB, self.color_object[2]*hexRGB, self.color_object[3]*hexRGB))
        self.layout.object_color_input.setStyleSheet(color1)
        # Live Intensity
        color1 = self.color_object[1]*self.color_object[4]*hexRGB
        color2 = self.color_object[2]*self.color_object[4]*hexRGB
        color3 = self.color_object[3]*self.color_object[4]*hexRGB
        # Limit Intensity
        if color1 < 0:
            color1 = 0
        if color1 > hexRGB:
        ...
        if color3 > hexRGB:
            color3 = hexRGB
        # Style Sheets
        color = str("background-color: rgb(%f, %f, %f);" % (color1, color2, color3))
        self.layout.object_color_intensity.setStyleSheet(color)
        # Signals
        self.Signal_Shader()
        self.Settings_Save()
    def Shader_Object_CLEAN(self, SIGNAL_CLEAN):
        self.color_object = ["False", 0, 0, 0, 1]
        self.Signal_Shader()
        self.Settings_Save()

    def Shader_Light_APPLY(self, SIGNAL_APPLY):
        if self.color_light[0] == "True":
            self.Color_RGB(self.color_light[1], self.color_light[2], self.color_light[3])
            self.Pigment_Display_Release(0)
            self.UVD_Release()
    def Shader_Light_SAVE(self, SIGNAL_SAVE):
        # Background
        self.layout.light_color.setStyleSheet("background-color: rgba(56, 56, 56, 255);")
        # Save Values
        self.color_light = ["True", self.rgb_1, self.rgb_2, self.rgb_3, self.layout.light_value.value()]
        # Input
        color1 = str("background-color: rgb(%f, %f, %f);" % (self.color_light[1]*hexRGB, self.color_light[2]*hexRGB, self.color_light[3]*hexRGB))
        self.layout.light_color_input.setStyleSheet(color1)
        # Live Intensity
        color1 = self.color_light[1]*self.color_light[4]*hexRGB
        color2 = self.color_light[2]*self.color_light[4]*hexRGB
        color3 = self.color_light[3]*self.color_light[4]*hexRGB
        # Limit Intensity
        if color1 < 0:
            color1 = 0
        if color1 > hexRGB:
        ...
        # Style Sheets
        color = str("background-color: rgb(%f, %f, %f);" % (color1, color2, color3))
        self.layout.light_color_intensity.setStyleSheet(color)
        # Signals
        self.Signal_Shader()
        self.Settings_Save()
    def Shader_Light_CLEAN(self, SIGNAL_CLEAN):
        self.color_light = ["False", 0, 0, 0, 1]
        self.Signal_Shader()
        self.Settings_Save()

    def Shader_Ambient_APPLY(self, SIGNAL_APPLY):
        if self.color_ambient[0] == "True":
            self.Color_RGB(self.color_ambient[1], self.color_ambient[2], self.color_ambient[3])
            self.Pigment_Display_Release(0)
            self.UVD_Release()
    def Shader_Ambient_SAVE(self, SIGNAL_SAVE):
        # Background
        self.layout.ambient_color.setStyleSheet("background-color: rgba(56, 56, 56, 255);")
        # Save Values
        self.color_ambient = ["True", self.rgb_1, self.rgb_2, self.rgb_3, self.layout.ambient_value.value()]
        # Input
        color1 = str("background-color: rgb(%f, %f, %f);" % (self.color_ambient[1]*hexRGB, self.color_ambient[2]*hexRGB, self.color_ambient[3]*hexRGB))
        self.layout.ambient_color_input.setStyleSheet(color1)
        # Live Intensity
        color1 = self.color_ambient[1]*self.color_ambient[4]*hexRGB
        color2 = self.color_ambient[2]*self.color_ambient[4]*hexRGB
        color3 = self.color_ambient[3]*self.color_ambient[4]*hexRGB
        # Limit Intensity
        if color1 < 0:
            color1 = 0
        if color1 > hexRGB:
        ...
        # Style Sheets
        color = str("background-color: rgb(%f, %f, %f);" % (color1, color2, color3))
        self.layout.ambient_color_intensity.setStyleSheet(color)
        # Signals
        self.Signal_Shader()
        self.Settings_Save()
    def Shader_Ambient_CLEAN(self, SIGNAL_CLEAN):
        self.color_ambient = ["False", 0, 0, 0, 1]
        self.Signal_Shader()
        self.Settings_Save()

    def Shader_Background_APPLY(self, SIGNAL_APPLY):
        if self.color_background[0] == "True":
            self.Color_RGB(self.color_background[1], self.color_background[2], self.color_background[3])
            self.Pigment_Display_Release(0)
            self.UVD_Release()
    def Shader_Background_SAVE(self, SIGNAL_SAVE):
        # Background
        self.layout.background_color.setStyleSheet("background-color: rgba(56, 56, 56, 255);")
        # Save Values
        self.color_background = ["True", self.rgb_1, self.rgb_2, self.rgb_3, self.layout.background_value.value()]
        # Input
        color1 = str("background-color: rgb(%f, %f, %f);" % (self.color_background[1]*hexRGB, self.color_background[2]*hexRGB, self.color_background[3]*hexRGB))
        self.layout.background_color_input.setStyleSheet(color1)
        # Live Intensity
        color1 = self.color_background[1]*hexRGB
        color2 = self.color_background[2]*hexRGB
        color3 = self.color_background[3]*hexRGB
        # Style Sheets
        color = str("background-color: rgba(%f, %f, %f, %f);" % (color1, color2, color3, self.sha_4))
        self.layout.background_color_intensity.setStyleSheet(color)
        # Signals
        self.Signal_Shader()
        self.Settings_Save()
    def Shader_Background_CLEAN(self, SIGNAL_CLEAN):
        self.color_background = ["False", 0, 0, 0, 1]
        self.Signal_Shader()
        self.Settings_Save()

    def Object_Slider(self, SIGNAL_VALUE):
        # Set Values
        self.sha_1 = SIGNAL_VALUE
        self.layout.object_value.setValue(self.sha_1*kritaOBJ)
        # Live Intensity
        color1 = self.color_object[1]*self.sha_1*kritaOBJ*hexRGB
        color2 = self.color_object[2]*self.sha_1*kritaOBJ*hexRGB
        color3 = self.color_object[3]*self.sha_1*kritaOBJ*hexRGB
        # Limit Intensity
        if color1 < 0:
            color1 = 0
        if color1 > hexRGB:
        ...
        self.Signal_Shader()
        self.Settings_Save()
    def Light_Slider(self, SIGNAL_VALUE):
        # Set Values
        self.sha_2 = SIGNAL_VALUE
        self.layout.light_value.setValue(self.sha_2*kritaLIG)
        # Live Intensity
        color1 = self.color_light[1]*self.sha_2*kritaLIG*hexRGB
        color2 = self.color_light[2]*self.sha_2*kritaLIG*hexRGB
        color3 = self.color_light[3]*self.sha_2*kritaLIG*hexRGB
        # Limit Intensity
        if color1 < 0:
            color1 = 0
        if color1 > hexRGB:
        ...
        self.Signal_Shader()
        self.Settings_Save()
    def Ambient_Slider(self, SIGNAL_VALUE):
        # Set Values
        self.sha_3 = SIGNAL_VALUE
        self.layout.ambient_value.setValue(self.sha_3*kritaAMB)
        # Live Intensity
        color1 = self.color_ambient[1]*self.sha_3*kritaAMB*hexRGB
        color2 = self.color_ambient[2]*self.sha_3*kritaAMB*hexRGB
        color3 = self.color_ambient[3]*self.sha_3*kritaAMB*hexRGB
        # Limit Intensity
        if color1 < 0:
            color1 = 0
        if color1 > hexRGB:
        ...
        self.Signal_Shader()
        self.Settings_Save()
    def Background_Slider(self, SIGNAL_VALUE):
        # Set Values
        self.sha_4 = SIGNAL_VALUE
        self.layout.background_value.setValue(self.sha_4)
        # Live Intensity
        color1 = self.color_background[1]*hexRGB
        color2 = self.color_background[2]*hexRGB
        color3 = self.color_background[3]*hexRGB
    ...
    def Object_Value(self):
        # Set Values
        self.sha_1 = self.layout.object_value.value() / kritaOBJ
        self.object_slider.Update(self.sha_1, self.layout.object_slider.width())
        # Live Intensity
        color1 = self.color_object[1]*self.sha_1*kritaOBJ*hexRGB
        color2 = self.color_object[2]*self.sha_1*kritaOBJ*hexRGB
    ...
    def Light_Value(self):
        # Set Values
        self.sha_2 = self.layout.light_value.value() / kritaLIG
        self.light_slider.Update(self.sha_2, self.layout.light_slider.width())
        # Live Intensity
        color1 = self.color_light[1]*self.sha_2*kritaLIG*hexRGB
        color2 = self.color_light[2]*self.sha_2*kritaLIG*hexRGB
    ...
    def Ambient_Value(self):
        # Set Values
        self.sha_3 = self.layout.ambient_value.value() / kritaAMB
        self.ambient_slider.Update(self.sha_3, self.layout.ambient_slider.width())
        # Live Intensity
        color1 = self.color_ambient[1]*self.sha_3*kritaAMB*hexRGB
        color2 = self.color_ambient[2]*self.sha_3*kritaAMB*hexRGB
    ...
    def Background_Value(self):
        # Set Values
        self.sha_4 = self.layout.background_value.value()
        self.background_slider.Update(self.sha_4, self.layout.background_slider.width())
        # Live Intensity
        color1 = self.color_background[1]*hexRGB
        color2 = self.color_background[2]*hexRGB
    ...
    def Shader_Output_1_APPLY(self, SIGNAL_APPLY):
        self.layout.object_live.setChecked(0)
        self.layout.light_live.setChecked(0)
        self.layout.ambient_live.setChecked(0)
        self.Color_RGB(self.output_1[0], self.output_1[1], self.output_1[2])
        self.Pigment_Display_Release(0)
        self.UVD_Release()
    def Shader_Output_2_APPLY(self, SIGNAL_APPLY):
        self.layout.object_live.setChecked(0)
        self.layout.light_live.setChecked(0)
        self.layout.ambient_live.setChecked(0)
        self.Color_RGB(self.output_2[0], self.output_2[1], self.output_2[2])
        self.Pigment_Display_Release(0)
        self.UVD_Release()
    def Shader_Output_3_APPLY(self, SIGNAL_APPLY):
        self.layout.object_live.setChecked(0)
        self.layout.light_live.setChecked(0)
        self.layout.ambient_live.setChecked(0)
        self.Color_RGB(self.output_3[0], self.output_3[1], self.output_3[2])
        self.Pigment_Display_Release(0)
        self.UVD_Release()
    def Shader_Output_4_APPLY(self, SIGNAL_APPLY):
        self.layout.object_live.setChecked(0)
        self.layout.light_live.setChecked(0)
        self.layout.ambient_live.setChecked(0)
        self.Color_RGB(self.output_4[0], self.output_4[1], self.output_4[2])
        self.Pigment_Display_Release(0)
        self.UVD_Release()
    def Shader_Output_5_APPLY(self, SIGNAL_APPLY):
        self.layout.object_live.setChecked(0)
        self.layout.light_live.setChecked(0)
        self.layout.ambient_live.setChecked(0)
        self.Color_RGB(self.output_5[0], self.output_5[1], self.output_5[2])
        self.Pigment_Display_Release(0)
        self.UVD_Release()

    def Signal_Shader(self):
        if self.color_object[0] != "True":
            self.layout.object_color.setStyleSheet(bg_alpha)
            self.layout.object_color_input.setStyleSheet(bg_unseen)
            self.layout.object_color_intensity.setStyleSheet(bg_unseen)
        if self.color_light[0] != "True":
            self.layout.light_color.setStyleSheet(bg_alpha)
            self.layout.light_color_input.setStyleSheet(bg_unseen)
            self.layout.light_color_intensity.setStyleSheet(bg_unseen)
        if self.color_ambient[0] != "True":
            self.layout.ambient_color.setStyleSheet(bg_alpha)
            self.layout.ambient_color_input.setStyleSheet(bg_unseen)
            self.layout.ambient_color_intensity.setStyleSheet(bg_unseen)
        if self.color_background[0] != "True":
            self.layout.background_color.setStyleSheet(bg_alpha)
            self.layout.background_color_input.setStyleSheet(bg_unseen)
            self.layout.background_color_intensity.setStyleSheet(bg_unseen)
        if (self.color_object[0] == "True" and self.color_light[0] == "True" and self.color_ambient[0] == "True" and self.color_background[0] == "True"):
            # Shader Model
            self.output_1 = self.Shader_Model(0.530864197530864)
            self.output_2 = self.Shader_Model(0.783950617283951)
            self.output_3 = self.Shader_Model(1)
            self.output_4 = self.Shader_Model(1.14197530864198)
            self.output_5 = self.Shader_Model(1.19753086419753)

            # Change Color
            # array_sphere_diffuse = self.SVG_Sphere_Diffuse(self.output_1, self.output_2, self.output_3, self.output_4, self.output_5, [self.color_background[1], self.color_background[2], self.color_background[3]], self.layout.background_value.value())
            # self.sphere.load(array_sphere_diffuse)
            # Background
            self.layout.shader_display.setStyleSheet(str("background-color: rgba(%f,%f,%f,%f);" % (self.color_background[1]*hexRGB, self.color_background[2]*hexRGB, self.color_background[3]*hexRGB, self.sha_4)))
            # 1
            color_1 = str("background-color: rgb(%f, %f, %f); border: 1px solid rgba(56, 56, 56, 255) ;" % (self.output_1[0]*hexRGB, self.output_1[1]*hexRGB, self.output_1[2]*hexRGB))
            self.output_1_values.Setup_RGB(self.output_1[0]*kritaRGB, self.output_1[1]*kritaRGB, self.output_1[2]*kritaRGB)
            self.layout.output_1_display.setStyleSheet(color_1)
            self.layout.output_1_values.setStyleSheet(bg_alpha)
            # 2
            color_2 = str("background-color: rgb(%f, %f, %f); border: 1px solid rgba(56, 56, 56, 255) ;" % (self.output_2[0]*hexRGB, self.output_2[1]*hexRGB, self.output_2[2]*hexRGB))
            self.output_2_values.Setup_RGB(self.output_2[0]*kritaRGB, self.output_2[1]*kritaRGB, self.output_2[2]*kritaRGB)
            self.layout.output_2_display.setStyleSheet(color_2)
            self.layout.output_2_values.setStyleSheet(bg_alpha)
            # 3
            color_3 = str("background-color: rgb(%f, %f, %f); border: 1px solid rgba(56, 56, 56, 255) ;" % (self.output_3[0]*hexRGB, self.output_3[1]*hexRGB, self.output_3[2]*hexRGB))
            self.output_3_values.Setup_RGB(self.output_3[0]*kritaRGB, self.output_3[1]*kritaRGB, self.output_3[2]*kritaRGB)
            self.layout.output_3_display.setStyleSheet(color_3)
            self.layout.output_3_values.setStyleSheet(bg_alpha)
            # 4
            color_4 = str("background-color: rgb(%f, %f, %f); border: 1px solid rgba(56, 56, 56, 255) ;" % (self.output_4[0]*hexRGB, self.output_4[1]*hexRGB, self.output_4[2]*hexRGB))
            self.output_4_values.Setup_RGB(self.output_4[0]*kritaRGB, self.output_4[1]*kritaRGB, self.output_4[2]*kritaRGB)
            self.layout.output_4_display.setStyleSheet(color_4)
            self.layout.output_4_values.setStyleSheet(bg_alpha)
            # 5
            color_5 = str("background-color: rgb(%f, %f, %f); border: 1px solid rgba(56, 56, 56, 255) ;" % (self.output_5[0]*hexRGB, self.output_5[1]*hexRGB, self.output_5[2]*hexRGB))
            self.output_5_values.Setup_RGB(self.output_5[0]*kritaRGB, self.output_5[1]*kritaRGB, self.output_5[2]*kritaRGB)
            self.layout.output_5_display.setStyleSheet(color_5)
            self.layout.output_5_values.setStyleSheet(bg_alpha)
        else:
            # Setup
            self.output_1_values.Setup_RGB(0,0,0)
            self.output_2_values.Setup_RGB(0,0,0)
            self.output_3_values.Setup_RGB(0,0,0)
            self.output_4_values.Setup_RGB(0,0,0)
            self.output_5_values.Setup_RGB(0,0,0)
            # Style Sheet
            self.layout.output_1_display.setStyleSheet(bg_alpha)
            self.layout.output_2_display.setStyleSheet(bg_alpha)
            self.layout.output_3_display.setStyleSheet(bg_alpha)
            self.layout.output_4_display.setStyleSheet(bg_alpha)
            self.layout.output_5_display.setStyleSheet(bg_alpha)
            self.layout.output_1_values.setStyleSheet(bg_alpha)
            self.layout.output_2_values.setStyleSheet(bg_alpha)
            self.layout.output_3_values.setStyleSheet(bg_alpha)
            self.layout.output_4_values.setStyleSheet(bg_alpha)
            self.layout.output_5_values.setStyleSheet(bg_alpha)
    def Shader_Model(self, factor):
        s1 = self.sha_1*2
        s2 = self.sha_2*2
        s3 = self.sha_3*2
        factor = factor * s2
        a = (self.color_object[1] * s1) * ((0.707 * self.color_light[1] * s2) + (self.color_ambient[1] * s3)) * factor
        b = (self.color_object[2] * s1) * ((0.707 * self.color_light[2] * s2) + (self.color_ambient[2] * s3)) * factor
        c = (self.color_object[3] * s1) * ((0.707 * self.color_light[3] * s2) + (self.color_ambient[3] * s3)) * factor
        # 0.707
        if a <= 0:
            a = 0
        if a >= 1:
    ...
    def Intensity_Load(self, object, light, ambient, background):
        # Block signals
        self.layout.object_slider.blockSignals(True)
        self.layout.object_value.blockSignals(True)
        self.layout.light_slider.blockSignals(True)
        self.layout.light_value.blockSignals(True)
        self.layout.ambient_slider.blockSignals(True)
        self.layout.ambient_value.blockSignals(True)
        self.layout.background_slider.blockSignals(True)
        self.layout.background_value.blockSignals(True)
        # Apply Values
        self.object_slider.Update(object, self.layout.object_slider.width())
        self.light_slider.Update(light, self.layout.light_slider.width())
        self.ambient_slider.Update(ambient, self.layout.ambient_slider.width())
        self.background_slider.Update(background, self.layout.background_slider.width())
        self.layout.object_value.setValue(object*kritaOBJ)
        self.layout.light_value.setValue(light*kritaLIG)
        self.layout.ambient_value.setValue(ambient*kritaAMB)
        self.layout.background_value.setValue(background*kritaBGC)
        # UnBlock Signals
        self.layout.object_slider.blockSignals(False)
        self.layout.object_value.blockSignals(False)
        self.layout.light_slider.blockSignals(False)
        self.layout.light_value.blockSignals(False)
        self.layout.ambient_slider.blockSignals(False)
        self.layout.ambient_value.blockSignals(False)
        self.layout.background_slider.blockSignals(False)
        self.layout.background_value.blockSignals(False)
    ...
    def Percentage(self, count):
        ...
        if self.count == "3":
            ...
            "stop:0.332 rgba(0, 0, 0, 0), \n"+
            "stop:0.333 rgba(0, 0, 0, 50), \n"+
            "stop:0.666 rgba(0, 0, 0, 50), \n"+
            "stop:0.667 rgba(0, 0, 0, 0), \n"+
    ...
    def Settings_Load(self):
        ...
        # Shader
        color_object_string = Krita.instance().readSetting("Pigment.O", "Shader_Object", "RGBA,U8,sRGB-elle-V2-srgbtrc.icc,1,0.8,0.4,1|RGBA,U8,sRGB-elle-V2-srgbtrc.icc,0,0,0,1")
        color_light_string = Krita.instance().readSetting("Pigment.O", "Shader_Light", "RGBA,U8,sRGB-elle-V2-srgbtrc.icc,1,0.8,0.4,1|RGBA,U8,sRGB-elle-V2-srgbtrc.icc,0,0,0,1")
        color_ambient_string = Krita.instance().readSetting("Pigment.O", "Shader_Ambient", "RGBA,U8,sRGB-elle-V2-srgbtrc.icc,1,0.8,0.4,1|RGBA,U8,sRGB-elle-V2-srgbtrc.icc,0,0,0,1")
        color_background_string = Krita.instance().readSetting("Pigment.O", "Shader_Background", "RGBA,U8,sRGB-elle-V2-srgbtrc.icc,1,0.8,0.4,1|RGBA,U8,sRGB-elle-V2-srgbtrc.icc,0,0,0,1")
        color_object_split = color_object_string.split(",")
        color_light_split = color_light_string.split(",")
        color_ambient_split = color_ambient_string.split(",")
        color_background_split = color_background_string.split(",")
        if color_object_split[0] == "True":
            # Color
            self.sha_1 = float(color_object_split[4])
            self.layout.object_color.setStyleSheet("background-color: rgba(56, 56, 56, 255) ;")
            self.color_object = ["True", float(color_object_split[1]), float(color_object_split[2]), float(color_object_split[3]), self.sha_1]
            color1 = str("background-color: rgb(%f, %f, %f);" % (self.color_object[1]*hexRGB, self.color_object[2]*hexRGB, self.color_object[3]*hexRGB))
            color2 = str("background-color: rgb(%f, %f, %f);" % (self.color_object[1]*self.sha_1*kritaOBJ*hexRGB, self.color_object[2]*self.sha_1*kritaOBJ*hexRGB, self.color_object[3]*self.sha_1*kritaOBJ*hexRGB))
            self.layout.object_color_input.setStyleSheet(color1)
            self.layout.object_color_intensity.setStyleSheet(color2)
        else:
            self.sha_1 = 1
            self.color_object = ["False", 0, 0, 0, 1]
            self.layout.object_color.setStyleSheet(bg_alpha)
            self.layout.object_color_input.setStyleSheet(bg_unseen)
            self.layout.object_color_intensity.setStyleSheet(bg_unseen)
        if color_light_split[0] == "True":
            # Color
            self.sha_2 = float(color_light_split[4])
            self.layout.light_color.setStyleSheet("background-color: rgba(56, 56, 56, 255) ;")
            self.color_light = ["True", float(color_light_split[1]), float(color_light_split[2]), float(color_light_split[3]), self.sha_2]
            color1 = str("background-color: rgb(%f, %f, %f);" % (self.color_light[1]*hexRGB, self.color_light[2]*hexRGB, self.color_light[3]*hexRGB))
            color2 = str("background-color: rgb(%f, %f, %f);" % (self.color_light[1]*self.sha_2*kritaLIG*hexRGB, self.color_light[2]*self.sha_2*kritaLIG*hexRGB, self.color_light[3]*self.sha_2*kritaLIG*hexRGB))
            self.layout.light_color_input.setStyleSheet(color1)
            self.layout.light_color_intensity.setStyleSheet(color2)
        else:
            self.sha_2 = 1
            self.color_light = ["False", 0, 0, 0, 1]
            self.layout.light_color.setStyleSheet(bg_alpha)
            self.layout.light_color_input.setStyleSheet(bg_unseen)
            self.layout.light_color_intensity.setStyleSheet(bg_unseen)
        if color_ambient_split[0] == "True":
            # Color
            self.sha_3 = float(color_ambient_split[4])
            self.layout.ambient_color.setStyleSheet("background-color: rgba(56, 56, 56, 255) ;")
            self.color_ambient = ["True", float(color_ambient_split[1]), float(color_ambient_split[2]), float(color_ambient_split[3]), self.sha_3]
            color1 = str("background-color: rgb(%f, %f, %f);" % (self.color_ambient[1]*hexRGB, self.color_ambient[2]*hexRGB, self.color_ambient[3]*hexRGB))
            color2 = str("background-color: rgb(%f, %f, %f);" % (self.color_ambient[1]*self.sha_3*kritaAMB*hexRGB, self.color_ambient[2]*self.sha_3*kritaAMB*hexRGB, self.color_ambient[3]*self.sha_3*kritaAMB*hexRGB))
            self.layout.ambient_color_input.setStyleSheet(color1)
            self.layout.ambient_color_intensity.setStyleSheet(color2)
        else:
            self.sha_3 = 1
            self.color_ambient = ["False", 0, 0, 0, 1]
            self.layout.ambient_color.setStyleSheet(bg_alpha)
            self.layout.ambient_color_input.setStyleSheet(bg_unseen)
            self.layout.ambient_color_intensity.setStyleSheet(bg_unseen)
        if color_background_split[0] == "True":
            # Color
            self.sha_4 = float(color_background_split[4])
            self.layout.background_color.setStyleSheet("background-color: rgba(56, 56, 56, 255) ;")
            self.color_background = ["True", float(color_background_split[1]), float(color_background_split[2]), float(color_background_split[3]), self.sha_4]
            color1 = str("background-color: rgb(%f, %f, %f);" % (self.color_background[1]*hexRGB, self.color_background[2]*hexRGB, self.color_background[3]*hexRGB))
            color2 = str("background-color: rgb(%f, %f, %f);" % (self.color_background[1]*self.sha_4*kritaBGC*hexRGB, self.color_background[2]*self.sha_4*kritaBGC*hexRGB, self.color_background[3]*self.sha_4*kritaBGC*hexRGB))
            self.layout.background_color_input.setStyleSheet(color1)
            self.layout.background_color_intensity.setStyleSheet(color2)
        else:
            self.sha_4 = 1
            self.color_background = ["False", 0, 0, 0, 1]
            self.layout.background_color.setStyleSheet(bg_alpha)
            self.layout.background_color_input.setStyleSheet(bg_unseen)
            self.layout.background_color_intensity.setStyleSheet(bg_unseen)
        # Adjust Slider Intensity
        self.Intensity_Load(self.sha_1, self.sha_2, self.sha_3, self.sha_4)
        self.Signal_Shader()
    ...
    def Settings_Save(self):
        try:
            ...
            # Shader
            color_object_list = (str(self.color_object[0]), str(self.color_object[1]), str(self.color_object[2]), str(self.color_object[3]), str(self.layout.object_value.value()/kritaOBJ))
            color_light_list = (str(self.color_light[0]), str(self.color_light[1]), str(self.color_light[2]), str(self.color_light[3]), str(self.layout.light_value.value()/kritaLIG))
            color_ambient_list = (str(self.color_ambient[0]), str(self.color_ambient[1]), str(self.color_ambient[2]), str(self.color_ambient[3]), str(self.layout.ambient_value.value()/kritaAMB))
            color_background_list = (str(self.color_background[0]), str(self.color_background[1]), str(self.color_background[2]), str(self.color_background[3]), str(self.layout.background_value.value()/kritaBGC))
            color_object_string = ','.join(color_object_list)
            color_light_string = ','.join(color_light_list)
            color_ambient_string = ','.join(color_ambient_list)
            color_background_string = ','.join(color_background_list)
            Krita.instance().writeSetting("Pigment.O", "Shader_Object", color_object_string)
            Krita.instance().writeSetting("Pigment.O", "Shader_Light", color_light_string)
            Krita.instance().writeSetting("Pigment.O", "Shader_Ambient", color_ambient_string)
            Krita.instance().writeSetting("Pigment.O", "Shader_Background", color_background_string)
