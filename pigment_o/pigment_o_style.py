...
class Style():
        ...
        for i in range(width):
            ...
            if cond1 <= cond2:
                ...
                if h <= 0:
                    ...
                    pass
            h = h / factorHUE
            s = (left[1] + (stop * cond3)) / factorSVL
            v = (left[2] + (stop * cond4)) / factorSVL

            # HSV to RGB Conversion
            rgb = self.convert.hsv_to_rgb(h, s, v)
            r = round(rgb[0]*factorRGB,3)
            g = round(rgb[1]*factorRGB,3)
            b = round(rgb[2]*factorRGB,3)
            # String
            slider_gradient += "stop:%s rgb(%s, %s, %s), \n " % (stop, r, g, b)
        slider_gradient += "stop:%s rgb(%s, %s, %s) ) ; \n " % (1.000, color_right[0], color_right[1], color_right[2])
        slider_gradient += "border: 1px solid rgba(56, 56, 56, 255) ; "
    ...
    def HSL_Gradient(self, width, color_left, color_right):
        ...
        for i in range(width):
            ...
            rgb = self.convert.hsl_to_rgb(h, s, lig)
            r = round(rgb[0]*factorRGB,3)
            g = round(rgb[1]*factorRGB,3)
            b = round(rgb[2]*factorRGB,3)
            # String
